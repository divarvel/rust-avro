...
pub fn encode<W: Write>(writer: &mut W, schema: &Schema, value: &Value) -> Result<(), EncodeError> {
    match (schema, value) {
        ...
        (&Schema::Boolean, &Value::Boolean(true))  => { writer.write(&[0x01]); Ok(()) },
        (&Schema::Boolean, &Value::Boolean(false)) => { writer.write(&[0x00]); Ok(()) },
        ...
        (&Schema::String, &Value::String(ref string)) => {
            encode(writer, &Schema::Long, &Value::Long(string.len() as i64));
        ...
        (&Schema::Enum(ref inner_schema), &Value::Enum(ref value_schema, num)) => {
            encode(writer, &Schema::Int, &Value::Int(num as i32));
            Ok(())
        },
